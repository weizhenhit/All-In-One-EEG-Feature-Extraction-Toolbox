...
%   此时，每个trial的特征矩阵表示为(2m*N, segments)
...
% Return
%       CSPMatrix: cell，学习到的CSP滤波器矩阵，共{nbClasses, nsegments}个，每个都是(channels * channels)
%       feature_train: 
%           feature_train.data: 训练集对应的CSP特征矩阵，(trials, 2m*nbClasses, nsegments)
%           feature_train.y: 即EEG_train.y
%       feature_test: 
%           feature_test.data: 测试集对应的CSP特征矩阵，(trials, 2m*nbClasses, nsegments)
%           feature_test.y: 即EEG_test.y
% called function：func_extractCSPFeatures_seg
% See also
%       feat_MulticlassCSP1v1_seg, feat_MulticlassRCSP1v1_seg,
...
function [feature_train, feature_test, CSPMatrix] = feat_MulticlassCSP1vR_seg(EEG_train, EEG_test, opts)
    fs = opts.fs;
    m = opts.m;
    ...
    % 只有两类：当前类与剩余类    
    if nbClasses == 2
        CSPMatrix = cell(1, nsegments);
        feature_train_data = zeros(nbTrials, 2*m, nsegments);   % 每个trial的特征矩阵为（2*m, nsegments）即每个样本可看做2维而非1维
        feature_test_data = zeros(size(EEG_test.x,3), 2*m, nsegments);   % 每个trial的label
    else
        CSPMatrix = cell(nbClasses, nsegments);
        feature_train_data = zeros(nbTrials, 2*m*nbClasses, nsegments);     % 每个trial的特征矩阵为（2*m*nbClasses, nsegments）即每个样本可看做2维而非1维
        feature_test_data = zeros(size(EEG_test.x,3), 2*m*nbClasses, nsegments);
    end
    feature_train.y = EEG_train.y;      
    ...
    for c=1:nbClasses
        for seg = 1:nsegments
            ...
            covMatrices1 = mean(trialCov(:,:,EEG_train.y == classLabels(c),seg),3);    % 当前类
            covMatrices2 = mean(trialCov(:,:,EEG_train.y ~= classLabels(c),seg),3);    % 剩余类
            ...
            U1 = U1(:, egIndex);
            % 得到空间滤波器矩阵
            CSPMatrix{c, seg} = U1' * P;    % nbChannels * nbChannels
            % disp(size(CSPMatrix{c}));
        end
        %% 提取CSP特征  训练集和测试集都使用训练集中得到的CSP空间滤波器矩阵
        % feature_train_2m和feature_test_2m的shape都是(trials,2m,nsegments)
        feature_train_2m = func_extractCSPFeatures_seg(EEG_train,CSPMatrix(c, :),m,seg_len);     %提取训练集特征       
        feature_test_2m = func_extractCSPFeatures_seg(EEG_test,CSPMatrix(c, :),m,seg_len);       %提取测试集特征
        % disp(size(feature_train_2m));
        % disp(size(feature_test_2m));
        
        % feature_train_data和feature_test_data的shape都是(trials, 2m*nbClasses, nsegments)
        feature_train_data(:, (c-1)*2*m+1:c*2*m, :) = feature_train_2m;
        feature_test_data(:, (c-1)*2*m+1:c*2*m, :) = feature_test_2m;
        
        if nbClasses == 2
            % 如果只有2个类别，没必要算两遍
            feature_train.data = feature_train_data;    % 每个trial的特征矩阵为（2*m*nbClasses, nsegments）即每个样本可看做2维而非1维
