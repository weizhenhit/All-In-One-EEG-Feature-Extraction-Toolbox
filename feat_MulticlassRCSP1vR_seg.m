...
function [feature_train, feature_test, CSPMatrix] = feat_MulticlassRCSP1vR_seg(EEG_train, EEG_test, opts)
    fs = opts.fs;
    m = opts.m;
    ...
    if nbClasses == 2
        ...
        feature_train_data = zeros(nbTrials, 2*m, nsegments);
        feature_test_data = zeros(size(EEG_test.x,3), 2*m, nsegments); 
    else
    ...
    for c=1:nbClasses
        for seg = 1:nsegments
            ...
            covMatrices1 = covMatrices1 ./ sum(EEG_train.y == classLabels(c));      % 存疑，是否应该再除一遍
            ...
            U = U(:,egIndex);
            % 得到空间滤波器矩阵
            CSPMatrix{c, seg} = U';      % nbChannels * nbChannels
