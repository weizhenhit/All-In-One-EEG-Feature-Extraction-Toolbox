...
%% 参数方法-AutoRegression model-burg法
% 使用AR模型中的burg方法[pxx,f] = pwelch(x,window,noverlap,nfft,fs)进行功率谱估计
% 调用调用bandpower(pxx, f, [f_low, f_high], 'psd')提取相对average band power特征
% 由于相同order下的pburg和pyulear得到的pxx估计基本完全相同，所以只使用其中一种
...
% opts.order: order of the autoregressive model, default = 20（暂定：取决于数据集）
% return: 5频带和2Hz均分频带划分得到的（5+bandwidth/2）维特征
% See also:
%       feat_PSDPer, feat_PSDWelch
...
function PSD = feat_PSDArburg(X, opts)
    X = detrend(X);     % EEG频谱分析之前通常需要detrend去趋势
    if isfield(opts,'order')	% 指定
        order = opts.order; 
    else	% 未指定 default
        order = 20;
    end
    ...
    [pxx, f] = pburg(X,order,nfft,fs);       % [pxx,f] = pburg(x,order,nfft,fs)
    ...
    for num = 1:bands
        average_band_power_2(1, num) = bandpower(pxx, f, [(num-1)*2+1 num*2+1], 'psd');  % (1, bands=30) 
